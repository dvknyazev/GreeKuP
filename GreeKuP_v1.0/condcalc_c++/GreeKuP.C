...
{
	char* programname = "Mpicondcalcscalar";
			...
			for (a = 0; a < 3; a++)
			{
				...
				for (J = Jbeg[rank]; J < Jfin[rank] + 1; J++)
				{
					for (J1 = 0; J1 < NB_TOT; J1++)
					{
						...
						real(nablamatrelem_proc[J-Jbeg[rank]][J1][a]) = (float)convert_dbl_to_flt;
						nablaread.read((char*)&convert_dbl_to_flt, sizeof(double));
						imag(nablamatrelem_proc[J-Jbeg[rank]][J1][a]) = (float)convert_dbl_to_flt;
					}
				}
			...
			for (omegacounter = 0; omegacounter < omeganum; omegacounter++)
			{
				...
				for (J = Jbeg[rank]; J < Jfin[rank] + 1; J++)
				{
					for (J1 = 0; J1 < NB_TOT; J1++)
					{
						...
						{
							dividebyvalue = abs(real(CELTOT[0][K][J1])-real(CELTOT[0][K][J]));
